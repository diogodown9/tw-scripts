 ...
 * Funcionalidades:
 * - Busca por Jogador (todas as aldeias do jogador no mundo ou mapa visível)
 * - Busca por Tribo (todas as aldeias da tribo no mundo ou mapa visível)
 * - Filtro de Pontos Mínimos e Máximos (aplicável na busca e na lista)
 * - Filtro por Raio de Distância da aldeia atual
 * - Seleção em massa de Bárbaras e Inimigos
 * - Múltiplos formatos de exportação (Espaço, Linha, [coord], [claim], Tabela BBCode, Com ID)
 * - Ordenação inteligente (Distância da sua aldeia, Pontuação, Aleatório/Shuffle)
 * - Sincronização bidirecional (Colar coordenadas externas e marcar no mapa)
 * - Histórico de cliques com Desfazer (Undo)
 * - Interface arrastável, minimizável e elegante
 */
(function () {
    'use strict';

    if (game_data.screen !== 'map') {
        UI.InfoMessage('A redirecionar para o mapa...', 2000, 'info');
    ...
    // Coordenadas da aldeia atual do jogador (origem para cálculo de distância)
    const currentOrigin = {
    ...
    let selectedList = []; // Array de { id, coord, x, y, name, owner, ownerName, allyTag, points, dist }
    let historyStack = []; // Para função Desfazer (Undo)

    // Cache para dados do mundo (carregados sob demanda)
    const worldCache = {
        players: null,
        tribes: null,
        villages: null,
    ...
    // --- Funções Auxiliares e Carregamento de Dados do Mundo ---
    ...
    async function ensureWorldDataLoaded() {
        ...
        try {
            ...
            UI.SuccessMessage('Dados do mundo carregados com sucesso!', 2000);
            return true;
        } catch (err) {
            worldCache.loading = false;
    ...
    function getVillageDataByCoord(x, y) {
        ...
        if (v) {
            ...
            if (ownerId !== 0 && TWMap.players && TWMap.players[ownerId]) {
    ...
    // Interceptar clique no mapa
    const originalHandleClick = TWMap.map._handleClick;
    ...
    // --- BUSCA AVANÇADA POR JOGADOR OU TRIBO ---

    $('#ecpSearchBtn').on('click', async function () {
        const query = $('#ecpSearchInput').val().trim();
        if (!query) {
            UI.ErrorMessage('Por favor, digite o nome do jogador ou a tag da tribo!', 3000);
            return;
        }

        const searchType = $('#ecpSearchType').val(); // 'player' ou 'ally'
        ...
        if (isWorldScope) {
            ...
            if (searchType === 'player') {
                // Busca exata ou parcial pelo jogador
                ...
                if (!player) {
                    UI.ErrorMessage(`Jogador "${query}" não encontrado no mundo!`, 3500);
                    return;
                }
                targetPlayerIds = [player.id];
                targetEntityName = `Jogador "${player.name}"`;
            } else {
                // Busca pela tribo (por tag ou nome)
                ...
                if (!tribe) {
                    UI.ErrorMessage(`Tribo "${query}" não encontrada no mundo!`, 3500);
                    return;
                }
            ...
            // Filtrar aldeias do jogador/tribo
            let countAdded = 0;
            ...
            // Apenas no mapa carregado na tela
            let countAdded = 0;
    ...
    // Permitir dar Enter no input de busca
    ...
    // Selecionar Bárbaras Visíveis no mapa com os filtros
    ...
    // Selecionar Inimigos Visíveis no mapa com os filtros
    $('#ecpSelectPlayers').on('click', function () {
        ...
        for (const key in TWMap.villages) {
            ...
            const isPlayer = (v.owner && v.owner !== '0' && v.owner !== 0 && parseInt(v.owner, 10) !== parseInt(game_data.player.id, 10));
    ...
    // Sincronizar texto colado no campo de texto
    $('#ecpSyncBtn').on('click', function () {
        ...
        if (!matches || matches.length === 0) {
            UI.ErrorMessage('Nenhuma coordenada válida encontrada no texto!', 3000);
            return;
        }
    ...
    $('#ecpSortDistAsc').on('click', function () {
        ...
        refreshView();
        UI.SuccessMessage('Ordenado por proximidade (crescente)!', 1500);
    });
    ...
    $('#ecpSortDistDesc').on('click', function () {
        ...
        refreshView();
        UI.SuccessMessage('Ordenado por distância (decrescente)!', 1500);
    });
    ...
    $('#ecpSortPoints').on('click', function () {
        ...
        refreshView();
        UI.SuccessMessage('Ordenado por pontos (maior primeiro)!', 1500);
    });
    ...
    $('#ecpShuffle').on('click', function () {
        ...
        refreshView();
        UI.SuccessMessage('Lista embaralhada!', 1500);
    });
    ...
    $('#ecpUndoBtn').on('click', function () {
        if (historyStack.length > 0) {
            ...
            refreshView();
            UI.InfoMessage('Ação desfeita!', 1500);
        } else {
            UI.InfoMessage('Nada para desfazer!', 1500);
        }
    });
    ...
    $('#ecpCopyBtn').on('click', function () {
        ...
        if (!text) {
            UI.ErrorMessage('Nada para copiar!', 2000);
            return;
        }

        if (navigator.clipboard && window.isSecureContext) {
            navigator.clipboard.writeText(text).then(() => {
                UI.SuccessMessage('Copiado para a área de transferência!', 2500);
            }).catch(() => fallbackCopy());
        } else {
            fallbackCopy();
        }
        ...
        function fallbackCopy() {
            ...
            UI.SuccessMessage('Copiado com sucesso!', 2500);
        }
    });
    ...
    $('#ecpClearBtn').on('click', function () {
        ...
        $('[id^="map_village_"]').css('filter', 'none');
        refreshView();
        UI.SuccessMessage('Seleção limpa!', 2000);
    });
    ...
    $('#ecpClose').on('click', function () {
        ...
        UI.InfoMessage('Coletor de Coordenadas encerrado.', 2000);
    });
