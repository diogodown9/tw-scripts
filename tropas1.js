(async function () {
    ...
    style.innerHTML = `
        ...
        .tw-ov-container { overflow-y: auto; overflow-x: auto; flex-grow: 1; padding-right: 5px; border: 1px solid #2e3235; border-radius: 8px; background: #181a1b; }
        ...
        .tw-table th, .tw-table td { padding: 8px 6px; border-bottom: 1px solid #2a2d30; text-align: center; vertical-align: middle; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .tw-table th { 
        ...
        .tw-table tbody tr { transition: background 0.1s; background: transparent; }
        .tw-table tbody tr:hover { background: #26292c !important; }
        
        .tw-row-ataque { background-color: rgba(255, 60, 60, 0.16) !important; }
        .tw-row-defesa { background-color: rgba(60, 140, 255, 0.16) !important; }

        .tw-zero { color: #555; opacity: 0.35; font-weight: normal; }
    ...
    function renderOverviewPage(page) {
            ...
            <div class="tw-tab-content active">
                <div class="tw-ov-container">
                    <table class="tw-table">
                        <thead>
                            <tr>
                                <th style="text-align:left; width: 220px; padding-left: 12px;">Aldeia</th>
                                <th style="width: 175px;">Fazenda</th>`;
        unitConfigs.forEach(u => {
            if (!u.isHidden) html += `<th style="width: 48px;"><img src="${u.src}" style="vertical-align: middle; filter: drop-shadow(0 2px 2px rgba(0,0,0,0.5));"></th>`;
        });
        html += `           </tr>
                        </thead>
                        <tbody>`;
        ...
        currentVillages.forEach(v => {
            html += `<tr class="${v.rowClass}">
                <td style="text-align:left; font-weight:bold; color:#7fbfff; padding-left: 12px;">
