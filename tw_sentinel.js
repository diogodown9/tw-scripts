...
(async function () {
    ...
    function parseTwDateTime(str, serverTimeObj = null) {
        ...
        } else if (!cleanStr.includes('hoje') && !cleanStr.includes('today')) {
            const ptMonths = {'jan':0, 'fev':1, 'mar':2, 'abr':3, 'mai':4, 'jun':5, 'jul':6, 'ago':7, 'set':8, 'out':9, 'nov':10, 'dez':11};
            const textMonthMatch = cleanStr.match(/(\d{1,2})[\.\/\s]+([a-z]{3})[\.\/\s]+(\d{2,4})?/);
            if (textMonthMatch) {
                const day = parseInt(textMonthMatch[1], 10);
                const mon = ptMonths[textMonthMatch[2]];
                d.setDate(day);
                if (mon !== undefined) d.setMonth(mon);
                if (textMonthMatch[3]) {
                    const yr = parseInt(textMonthMatch[3], 10);
                    d.setFullYear(yr < 100 ? 2000 + yr : yr);
                }
            } else {
    ...
    function parseAcademyProduction(data, currentVillageId = null) {
        ...
        rowMatches.forEach(row => {
            ...
            let endtimeSec = 0;
            if (timerMatch) {
                if (/^\d{9,11}$/.test(timerMatch[1].trim())) {
                    endtimeSec = parseInt(timerMatch[1].trim(), 10);
                } else {
            ...
            const now = Date.now();
            let readyAtMs = now + 3600000;
            if (endtimeSec > 0) {
                readyAtMs = endtimeSec * 1000;
            } else if (completionStr) {
                readyAtMs = parseTwDateTime(completionStr) || (now + remainingSec * 1000);
            } else if (remainingSec > 0) {
                readyAtMs = now + remainingSec * 1000;
            }
            ...
            for (let i = 0; i < count; i++) {
                prods.push({
                    ...
                    coords,
                    type: 'production',
                    timerStr: timerStr || (remainingSec ? formatDuration(remainingSec) : ''),
                    remainingSec: remainingSec || Math.max(0, Math.floor((readyAtMs - now) / 1000)),
                    completionStr: completionStr || new Date(readyAtMs).toLocaleTimeString('pt-PT'),
                    readyAtMs
                });
            }
        });
    ...
    async function loadData() {
        ...
        try {
            ...
            const [rU, rP, rS, rSnobDirect, rSnobPopup, rSnobTrain, rVillageOverview, rBuildings] = await Promise.all([
            ...
            const villageSnobsInProdMap = {};
            const villagesWithAcademy = new Set();
            ...
            if (uTable) {
                ...
                Array.from(uTable.querySelectorAll('tbody')).forEach(tb => {
                    ...
                    const inProdFromOverview = villageSnobsInProdMap[vId] || 0;
                    const vObj = {
                        ...
                        snobsInProd: inProdFromOverview,
                        snobsReturning: 0,
                        noblePendingEvents: [],
                        hasSnobsAway: snobsOutside > 0 || inProdFromOverview > 0,
                    ...
                    if (inProdFromOverview > 0) {
                        allSnobProductions.push({
                            villageId: vId,
                            coords,
                            type: 'production',
                            timerStr: '',
                            remainingSec: 3600,
                            completionStr: '',
                            readyAtMs: Date.now() + 3600000,
                            note: 'Nobre em treino na Academia'
                        });
                    }
                });
            } else {
            ...
            const academyPromise = (academyCheckIds.length > 0)
                ? Promise.all(academyCheckIds.map(async (vId) => {
                    try {
                        ...
                        if (html) {
                            const prods = parseAcademyProduction(html, vId);
    ...
    function renderAttackPlanner() {
                                ...
                                <div style="display:none; grid-template-columns: 1fr 1fr; gap:4px;" id="tw-box-batevolta-options">
                                    ...
                                    <div style="display:flex; flex-direction:column; gap:1px;" id="tw-box-batevolta-buffer">
                                        ...
                                        <select id="tw-nt-bv-buffer" class="tw-select" style="padding:4px 6px; font-size:11px; font-weight:bold; color:#38bdf8;">
											<option value="75" ${bvBufferPref==='75'?'selected':''}>75s (Seguro PSEvolution)</option>
											<option value="90" ${bvBufferPref==='90'?'selected':''}>90s (Recomendado)</option>
											<option value="120" ${bvBufferPref==='120'?'selected':''}>120s (2 min)</option>
											<option value="60" ${bvBufferPref==='60'?'selected':''}>60s (Mínimo)</option>
										</select>
                                    </div>
                                </div>
                            </div>
        ...
        function updateNobleProximityHUD(preferredPrimaryId = null, isUserManualChange = false) {
            ...
            if (nobleVillages.length === 0) {
                if (hintBox) {
                    hintBox.innerHTML = '❌ <b style="color:#ef4444;">Nenhuma aldeia com nobres ou nobres em treino encontrada.</b>';
            ...
            // Função para gerar HTML das opções sem optgroups
            const renderNobleDropdownOptions = (isSecondary = false) => {
                return listWithDist.map((item, idx) => {
                    const v = item.village;
                    const pal = (v.paladin && v.paladin.isHome) ? v.paladin : null;
                    const palTag = pal ? ` [${pal.name}${pal.name === 'QuimConquista' ? ' ⚔️ Persuasão' : ''}]` : '';
                    const isClosest = (closest && v.id === closest.village.id);
                    const isBestWithEnough = (closestWithEnough && v.id === closestWithEnough.village.id);
                    
                    let prefix = `[#${idx + 1}] `;
                    if (!isSecondary && isBestWithEnough) {
                        prefix = item.hasReqNobles ? '⭐ [MAIS PERTO PRONTA] ' : '⚠️ [MAIS PERTO] ';
                    } else if (!isSecondary && isClosest && !isBestWithEnough) {
                        prefix = '⭐ [1º MAIS PERTO] ';
                    } else if (isSecondary && idx === 1) {
                        prefix = '⭐ [2º MAIS PERTO] ';
                    }
            ...
            selPrimary.innerHTML = renderNobleDropdownOptions(false);

            // Renderizar opções para Aldeia Nobres Secundária (Split 2x2)
            if (selSecondary) {
                selSecondary.innerHTML = renderNobleDropdownOptions(true);
            }
    ...
    function renderDefensePlanner() {
        ...
        const villageRiskMap = buildDefenseVillageRiskMap(threatRows);
        const movementPlan = buildDefenseMovementPlan(villageRiskMap);

        const comboBanner = combos.length ? `
            <div style="padding:10px 12px; border:1px solid #ef4444; border-radius:8px; background:linear-gradient(90deg, rgba(239,68,68,0.18) 0%, rgba(15,23,42,0.9) 100%); margin-bottom:2px;">
                <div style="display:flex; align-items:center; gap:8px; margin-bottom:6px;">
                    <span style="font-size:14px;">🚨</span>
                    <span style="font-weight:bold; color:#fca5a5; font-size:12px;">ALERTA DE COMBO ATIVO</span>
                ...
                ${combos.slice(0, 3).map(c => `
                    <div style="font-size:11px; color:#e2e8f0; padding:5px 0; border-top:1px dashed rgba(148,163,184,0.25);">
                        ...
                        ${escapeHtml(c.label)} — <b>${c.count}</b> ataques em ${c.spanMinutes}m • ${c.targets.length} alvo(s) • risco ${c.risk}/100
        ...
        const pressureCards = (ctx.playerPressure || []).slice(0, 5).map(p => {
            ...
            return `
                <div style="padding:8px 10px; border:1px solid ${p.risk >= 68 ? '#ef4444' : p.risk >= 46 ? '#f59e0b' : '#38bdf8'}; border-radius:7px; background:rgba(15,23,42,0.82); margin-bottom:6px;">
                    <div style="display:flex; justify-content:space-between; gap:8px; align-items:center;">
                        <span style="font-size:11px; color:#f8fafc; font-weight:700;">${escapeHtml(p.playerName)}</span>
                        <span style="font-size:10px; color:${p.risk >= 68 ? '#fca5a5' : p.risk >= 46 ? '#fbbf24' : '#7dd3fc'}; font-weight:bold;">${p.risk}/100</span>
                    </div>
                    <div style="font-size:10px; color:#cbd5e1; margin-top:4px;">${p.count} ataques • ${p.totalTargets} alvos</div>
                    <div style="font-size:10px; color:${autoColor}; margin-top:3px;">🤖 Automação ${auto.score}% <span style="color:#64748b;">(conf. ${auto.confidence}%)</span></div>
                    ${auto.reasons.length ? `<div style="font-size:9.5px; color:#64748b; margin-top:2px;">${escapeHtml(auto.reasons.slice(0, 3).join(' • '))}</div>` : ''}
                </div>
            `;
        }).join('');

        const threatCards = threatRows.length ? threatRows.slice(0, 10).map(t => {
            const auto = estimateDefenseAutomationConfidence(threatRows.filter(x => x.playerName === t.playerName));
            return `
                <div style="padding:10px 12px; border:1px solid ${t.risk >= 86 ? '#ef4444' : t.risk >= 68 ? '#f59e0b' : t.risk >= 46 ? '#fbbf24' : '#0ea5e9'}; border-radius:8px; background:rgba(15, 23, 42, 0.94); margin-bottom:8px;">
                    <div style="display:flex; justify-content:space-between; gap:8px; align-items:center; flex-wrap:wrap; margin-bottom:6px;">
                        <div>
                            <div style="font-size:11px; color:#94a3b8;">${escapeHtml(t.playerName)}</div>
                            <div style="font-weight:bold; color:#f8fafc; font-size:13px;">${escapeHtml(t.targetVillage ? t.targetVillage.name : (t.targetName || t.targetCoords || '—'))}</div>
                        </div>
                        <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap; justify-content:flex-end;">
                            <span style="padding:2px 7px; border-radius:999px; font-size:10px; font-weight:bold; background:${t.risk >= 86 ? 'rgba(239,68,68,0.18)' : t.risk >= 68 ? 'rgba(245,158,11,0.16)' : t.risk >= 46 ? 'rgba(251,191,36,0.12)' : 'rgba(14,165,233,0.15)'}; color:${t.risk >= 86 ? '#fca5a5' : t.risk >= 68 ? '#fbbf24' : '#7dd3fc'}; border:1px solid ${t.risk >= 86 ? '#ef4444' : t.risk >= 68 ? '#f59e0b' : '#0ea5e9'};">${t.risk}/100</span>
                            <span style="font-size:10px; color:#e2e8f0; font-weight:bold;">${t.recommendation.level}</span>
                        </div>
                    </div>
                    <div style="font-size:11px; line-height:1.5; color:#cbd5e1;">
                        <div><b>Origem:</b> ${escapeHtml(t.originCoords || '—')} • ${((t.originVillage && t.originVillage.points) || 0).toLocaleString('pt-PT')} pts</div>
                        <div><b>Impacto:</b> ${t.targetCoords || '—'} • <b>Janela:</b> ${t.sameWindow || 0} no slot • <b>Automação:</b> ${auto.score}%</div>
                        <div><b>Ação:</b> ${t.recommendation.action}</div>
                        <div style="color:#bfdbfe; margin-top:4px;">${t.recommendation.reason}</div>
                    </div>
                </div>
            `;
        }).join('') : `<div style="padding:24px; text-align:center; color:#94a3b8;">Nenhum ataque inimigo a chegar identificado para análise de defesa.</div>`;

        const villageCards = villageRiskMap.length ? villageRiskMap.slice(0, 8).map(v => `
            <div style="padding:8px 10px; border-left:3px solid ${v.exposure >= 70 ? '#ef4444' : v.exposure >= 46 ? '#f59e0b' : '#0ea5e9'}; border-radius:5px; background:rgba(15,23,42,0.8); margin-bottom:6px;">
                <div style="display:flex; justify-content:space-between; gap:6px; align-items:center;">
                    <span style="font-size:11px; color:#f8fafc; font-weight:700; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${escapeHtml(v.name)}</span>
                    <span style="font-size:10px; font-weight:bold; color:${v.exposure >= 70 ? '#fca5a5' : v.exposure >= 46 ? '#fbbf24' : '#7dd3fc'}; white-space:nowrap;">${v.exposure}/100</span>
                </div>
                <div style="font-size:10px; color:#cbd5e1; margin-top:3px;">${v.attackCount} ataque(s) • ${v.hasNoble ? '👑 com nobre • ' : ''}defesa ${v.defensePotential}/100</div>
                <div style="font-size:9.5px; color:#64748b; margin-top:2px;">${escapeHtml(v.players.join(', ') || '—')}${v.earliestMs ? ` • 1º: ${formatShortTime(v.earliestMs)}` : ''}</div>
            </div>
        `).join('') : '<div style="padding:16px; color:#64748b; text-align:center;">Nenhuma aldeia tua sob ameaça direta.</div>';

        const movementCards = movementPlan.length ? movementPlan.slice(0, 6).map(m => `
            <div style="padding:9px 11px; border:1px solid ${m.entry.exposure >= 70 ? 'rgba(239,68,68,0.4)' : 'rgba(56,189,248,0.35)'}; border-radius:7px; background:rgba(15,23,42,0.85); margin-bottom:6px;">
                <div style="display:flex; justify-content:space-between; gap:8px; align-items:center; flex-wrap:wrap;">
                    <span style="font-size:11px; color:#f8fafc; font-weight:700;">${escapeHtml(m.entry.name)} <span style="color:#64748b; font-weight:400;">(${escapeHtml(m.entry.coords)})</span></span>
                    <span style="font-size:9.5px; font-weight:bold; padding:2px 7px; border-radius:999px; background:rgba(56,189,248,0.15); color:#7dd3fc; border:1px solid rgba(56,189,248,0.3);">${m.action}</span>
                </div>
                <div style="font-size:10px; color:#cbd5e1; margin-top:4px;">Defesa em casa: <b>${m.entry.defensePotential}/100</b> • falta reforço <b style="color:#fbbf24;">~${m.needed}</b></div>
                <div style="font-size:10px; color:#bfdbfe; margin-top:3px;">${escapeHtml(m.note)}</div>
            </div>
        `).join('') : '<div style="padding:16px; color:#64748b; text-align:center;">Sem necessidade de movimentação defensiva no momento.</div>';

        document.getElementById('tw-main-body').innerHTML = `
            <div class="tw-pane active" style="padding:8px; gap:10px; display:flex; flex-direction:column; min-height:0;">
                <div class="tw-kpi-grid" style="grid-template-columns: repeat(5, 1fr);">
                    <div class="tw-kpi-card tw-kpi-red">
                    ...
                    <div class="tw-kpi-card tw-kpi-orange">
                        <div class="tw-kpi-label"><span>🧠 ALTO RISCO</span><span>≥68</span></div>
                        <div class="tw-kpi-value" style="color:#fbbf24;">${ctx.totalHighRisk || 0}</div>
                        <div class="tw-kpi-sub">Ameaças críticas</div>
                    </div>
                    <div class="tw-kpi-card tw-kpi-red">
                        <div class="tw-kpi-label"><span>🚨 COMBOS</span><span>JANELA</span></div>
                        <div class="tw-kpi-value" style="color:#fca5a5;">${combos.length}</div>
                        <div class="tw-kpi-sub">Pressão concentrada</div>
                    </div>
                    <div class="tw-kpi-card tw-kpi-blue">
                        <div class="tw-kpi-label"><span>🛡️ BUNKER</span><span>IA</span></div>
                        <div class="tw-kpi-value" style="color:#7dd3fc;">${movementPlan.length}</div>
                        <div class="tw-kpi-sub">Aldeias a reforçar</div>
                    </div>
                    <div class="tw-kpi-card tw-kpi-green">
                        <div class="tw-kpi-label"><span>✅ VIGILÂNCIA</span><span>OK</span></div>
                        <div class="tw-kpi-value" style="color:#34d399;">${threatRows.filter(t => t.risk < 46).length}</div>
                        <div class="tw-kpi-sub">Baixo risco</div>
                    </div>
                </div>

                ${comboBanner}

                <div class="tw-card" style="padding:10px 12px; display:flex; justify-content:space-between; gap:10px; align-items:center; flex-wrap:wrap;">
                    <div style="flex:1; min-width:260px;">
                        <div class="tw-card-title" style="color:#38bdf8; font-size:11px; margin-bottom:3px;">
                            <span>🧠 IA TÁTICA DE DEFESA AVANÇADA</span>
                        </div>
                        <div style="font-size:10.5px; color:#94a3b8; line-height:1.5;">
                            Modelo multi-fator: janelas de chegada, combo multi-alvo, pressão por jogador, OD aparente, prioridade estratégica da aldeia, defesa em casa e deteção de padrões de agendamento automático (PSEvolution-like).
                        </div>
                    </div>
                    <button class="tw-btn tw-btn-blue" id="tw-def-copy-advisory" style="padding:7px 14px; font-size:11px; font-weight:bold; white-space:nowrap;">📋 Copiar Aconselhamento</button>
                </div>

                <div style="display:grid; grid-template-columns: 1.2fr 1fr; gap:8px; min-height:0;">
                    <div class="tw-panel" style="padding:10px; overflow-y:auto; min-height:220px;">
                        <div style="font-size:10px; color:#94a3b8; font-weight:700; text-transform:uppercase; letter-spacing:0.05em; margin-bottom:8px;">Ameaças prioritárias</div>
                        ${threatCards}
                    </div>
                    <div style="display:flex; flex-direction:column; gap:8px; min-height:0;">
                        <div class="tw-panel" style="padding:10px; overflow-y:auto; min-height:0; flex:1;">
                            <div style="font-size:10px; color:#94a3b8; font-weight:700; text-transform:uppercase; letter-spacing:0.05em; margin-bottom:8px;">Aldeias sob risco</div>
                            ${villageCards}
                        </div>
                        <div class="tw-panel" style="padding:10px; overflow-y:auto; min-height:0; flex:1;">
                            ...
                            ${pressureCards || '<div style="padding:16px; color:#64748b; text-align:center;">Sem pressão relevante por jogador.</div>'}
                        </div>
                    </div>
                </div>

                <div class="tw-panel" style="padding:10px; overflow-y:auto; max-height:230px;">
                    <div style="font-size:10px; color:#94a3b8; font-weight:700; text-transform:uppercase; letter-spacing:0.05em; margin-bottom:8px;">🛡️ Plano de movimento sugerido</div>
                    ${movementCards}
                </div>
            </div>
        `;
        ...
        if (copyBtn) {
            copyBtn.onclick = () => {
                ...
                safeCopyText(text);
                showToast('📋 Aconselhamento de defesa copiado!');
            };
        }
    }
    ...
    // ==========================================
    // MÓDULO RADAR TÁTICO & INTEL DE JOGADOR
    // (Inspirado e aprimorado a partir de comandos5.js)
    // ==========================================
    let commandsSubTab = 'my_commands'; // 'my_commands' | 'radar_intel'
    let radarTargetPlayer = '';
    let radarTargetPlayerId = null;
    let radarDeepScan = true;
    let radarFilterSupport = false;
    let radarIsScanning = false;
    let radarScanAbort = false;
    let radarResults = [];
    let radarFilter = 'all'; // 'all' | 'noble' | 'nuke' | 'fake' | 'spy' | 'support'
    let radarSearch = '';
    let radarTimerInterval = null;
    const collapsedRadarGroups = new Set();

    const TW_UNIT_POP_MAP = {
        spear: 1, sword: 1, axe: 1, archer: 1, spy: 2, light: 4, marcher: 5, heavy: 6, ram: 5, catapult: 8, knight: 10, snob: 100
    };

    function parseTroopsFromHtmlOrDoc(source) {
        let found = false, noble = false, pop = 0, spyOnly = true;
        if (!source) return { found, noble, pop, spyOnly };
                        ...
                        if (img && dataCells[i]) {
                            ...
                            if (m) {
                                ...
                                if (!isNaN(val) && val > 0) {
                                    ...
                                    pop += val * (TW_UNIT_POP_MAP[unit] || 0);
                                }
                            }
                        }
                    });
    ...
    function classifyRadarThreat(cmdType, troops, commandName, htmlContext = '') {
        let badgeHtml = '';
        let classificationLabel = 'Desconhecido';
        let isPriority = false;
        let rank = 5;

        if (cmdType === 'Apoio') {
            classificationLabel = 'Apoio';
            badgeHtml = `<span style="background-color:#059669; color:#fff; font-weight:700; font-size:10px; padding:2px 7px; border-radius:4px; display:inline-flex; align-items:center; gap:3px;">🛡️ APOIO${troops.found && troops.pop > 0 ? ` <span style="opacity:0.85; font-size:9.5px;">(${troops.pop} pop)</span>` : ''}</span>`;
            rank = 5;
        } else {
            if (troops.found) {
                if (troops.noble) {
                    classificationLabel = 'Nobre';
                    badgeHtml = `<span style="background-color:#9333ea; color:#fff; font-weight:800; font-size:10px; padding:2px 7px; border-radius:4px; display:inline-flex; align-items:center; gap:3px; box-shadow:0 0 6px rgba(147,51,234,0.4);">👑 NOBRE${troops.pop > 0 ? ` <span style="opacity:0.85; font-size:9.5px;">(${troops.pop} pop)</span>` : ''}</span>`;
                    isPriority = true;
                    rank = 1;
                } else if (troops.spyOnly) {
                    classificationLabel = 'Espionagem';
                    badgeHtml = `<span style="background-color:#2563eb; color:#fff; font-weight:700; font-size:10px; padding:2px 7px; border-radius:4px; display:inline-flex; align-items:center; gap:3px;">👁️ ESPIONAGEM${troops.pop > 0 ? ` <span style="opacity:0.85; font-size:9.5px;">(${troops.pop} pop)</span>` : ''}</span>`;
                    rank = 4;
                } else if (troops.pop > 200) {
                    classificationLabel = 'Grande Escala';
                    badgeHtml = `<span style="background-color:#dc2626; color:#fff; font-weight:800; font-size:10px; padding:2px 7px; border-radius:4px; display:inline-flex; align-items:center; gap:3px; box-shadow:0 0 6px rgba(220,38,38,0.4);">⚔️ GRANDE ESCALA <span style="opacity:0.85; font-size:9.5px;">(${troops.pop} pop)</span></span>`;
                    isPriority = true;
                    rank = 2;
                } else {
                    classificationLabel = 'Fake';
                    badgeHtml = `<span style="background-color:#d97706; color:#fff; font-weight:700; font-size:10px; padding:2px 7px; border-radius:4px; display:inline-flex; align-items:center; gap:3px;">🗡️ FAKE <span style="opacity:0.85; font-size:9.5px;">(${troops.pop} pop)</span></span>`;
                    rank = 3;
                }
            } else {
                const combined = (commandName + ' ' + htmlContext).toLowerCase();
                if (combined.includes('snob') || combined.includes('nobre')) {
                    classificationLabel = 'Nobre';
                    badgeHtml = `<span style="background-color:#9333ea; color:#fff; font-weight:800; font-size:10px; padding:2px 7px; border-radius:4px; display:inline-flex; align-items:center; gap:3px; box-shadow:0 0 6px rgba(147,51,234,0.4);">👑 NOBRE</span>`;
                    isPriority = true;
                    rank = 1;
                } else if (combined.includes('attack_large') || combined.includes('attack_medium') || combined.includes('5000+')) {
                    classificationLabel = 'Grande Escala';
                    badgeHtml = `<span style="background-color:#dc2626; color:#fff; font-weight:800; font-size:10px; padding:2px 7px; border-radius:4px; display:inline-flex; align-items:center; gap:3px; box-shadow:0 0 6px rgba(220,38,38,0.4);">⚔️ GRANDE ESCALA</span>`;
                    isPriority = true;
                    rank = 2;
                } else if (combined.includes('attack_small')) {
                    classificationLabel = 'Fake';
                    badgeHtml = `<span style="background-color:#d97706; color:#fff; font-weight:700; font-size:10px; padding:2px 7px; border-radius:4px; display:inline-flex; align-items:center; gap:3px;">🗡️ FAKE</span>`;
                    rank = 3;
                } else if (combined.includes('spy') || combined.includes('batedor')) {
                    classificationLabel = 'Espionagem';
                    badgeHtml = `<span style="background-color:#2563eb; color:#fff; font-weight:700; font-size:10px; padding:2px 7px; border-radius:4px; display:inline-flex; align-items:center; gap:3px;">👁️ ESPIONAGEM</span>`;
                    rank = 4;
                } else if (/(viking|machado|cl|leve|nobre|ariete|catapulta|full|snip)/i.test(commandName)) {
                    classificationLabel = 'Alvo Detetado';
                    badgeHtml = `<span style="background-color:#b91c1c; color:#fff; font-weight:800; font-size:10px; padding:2px 7px; border-radius:4px; display:inline-flex; align-items:center; gap:3px;">⚔️ ALVO DETETADO</span>`;
                    isPriority = true;
                    rank = 2;
                } else {
                    classificationLabel = 'Oculto';
                    badgeHtml = `<span style="background-color:#475569; color:#cbd5e1; font-weight:600; font-size:10px; padding:2px 7px; border-radius:4px; display:inline-flex; align-items:center; gap:3px;">❓ OCULTO</span>`;
                    rank = 6;
                }
            }
        }
        return { badgeHtml, classificationLabel, isPriority, rank };
    }

    async function fetchDeepCommandInfo(partialCmd) {
        try {
            const res = await fetch(`/game.php?screen=info_command&id=${partialCmd.id}`);
            if (!res.ok) return null;
            const html = await res.text();
            const doc = new DOMParser().parseFromString(html, 'text/html');

            let origin = '-------', player = '-------', arrivalStr = '-', commandName = 'Comando';
            ...
            const isAttack = h2Html.includes('attack') || commandName.toLowerCase().includes('ataque') || !h2Html.includes('support');
            ...
            let troops = parseTroopsFromHtmlOrDoc(doc);
            if (!troops.found && partialCmd.tooltipTroops && partialCmd.tooltipTroops.found) {
                troops = partialCmd.tooltipTroops;
            }

            const classification = classifyRadarThreat(cmdType, troops, commandName, h2Html + ' ' + partialCmd.tooltipHtml);
            const endTime = parseTwDateTime(arrivalStr) || 0;

            let timerStr = '--:--:--';
            if (endTime > 0) {
                const nowSec = Math.floor(getTwServerTimeMs() / 1000);
            ...
            return {
                ...
                text: commandName,
                type: cmdType,
                badgeHtml: classification.badgeHtml,
                ...
                destCoord: partialCmd.destCoord,
    ...
    function startRadarTimer() {
        ...
        const timerEls = Array.from(document.querySelectorAll('.tw-radar-timer'));
        ...
        radarTimerInterval = setInterval(() => {
            ...
            timerEls.forEach(el => {
                const endSecStr = el.getAttribute('data-endtime-sec');
                if (!endSecStr) return;
                const endSec = parseInt(endSecStr, 10);
                if (!endSec || endSec <= 0) return;

                const rem = endSec - nowSec;
                if (rem <= 0) {
                    el.textContent = '0:00:00';
                    el.style.color = '#ef4444';
                    el.removeAttribute('data-endtime-sec');
                } else {
                    activeCount++;
                    const h = Math.floor(rem / 3600);
                    const m = Math.floor((rem % 3600) / 60);
            ...
            if (activeCount === 0 && radarTimerInterval) {
                clearInterval(radarTimerInterval);
                radarTimerInterval = null;
            }
        }, 1000);
    ...
    function generateRadarBbCode(results) {
        if (!results || results.length === 0) return '';
        let bb = `[b]📡 RELATÓRIO DE INTELIGÊNCIA / RADAR DE COMANDOS[/b]\n`;
        bb += `Alvo: [b]${radarTargetPlayer || 'Desconhecido'}[/b] | Total de Comandos: ${results.length}\n`;
        bb += `Gerado às: ${new Date(getTwServerTimeMs()).toLocaleTimeString('pt-PT')} (Servidor)\n\n`;
        bb += `[table]\n`;
        bb += `[**]Comando[||]Classificação[||]Origem[||]Jogador[||]Destino[||]Chegada[||]Restante[/**]\n`;
        results.forEach(c => {
            const clsClean = c.classificationLabel || c.type;
            const originBb = c.origin && c.origin !== '-------' ? `[coord]${c.origin}[/coord]` : '---';
            const destBb = c.destCoord ? `[coord]${c.destCoord}[/coord]` : (c.destination || '---');
            const playerBb = c.player && c.player !== '-------' ? `[player]${c.player}[/player]` : '---';
            const timerText = c.timerStr || '--:--:--';
            bb += `[*]${c.text || 'Comando'}[|]${clsClean}[|]${originBb}[|]${playerBb}[|]${destBb}[|]${c.arrivalStr || '-'}[|]${timerText}\n`;
        });
        bb += `[/table]\n`;
        return bb;
    }
    ...
    async function startRadarScan() {
        ...
        if (!worldVillagesLoaded || !worldPlayersLoaded) {
            await fetchWorldData();
        }

        if (!radarTargetPlayerId && radarTargetPlayer) {
            const lower = radarTargetPlayer.trim().toLowerCase();
            for (const [id, name] of Object.entries(worldPlayers)) {
                if (name.toLowerCase() === lower) {
                    radarTargetPlayerId = id;
                    radarTargetPlayer = name;
                    break;
                }
            }
        }

        if (!radarTargetPlayerId) {
            showToast('⚠️ Seleciona ou digita um jogador válido!');
            return;
        }

        let targetVillages = (worldVillages || []).filter(v => String(v.playerId) === String(radarTargetPlayerId));

        if (targetVillages.length === 0) {
            try {
                const res = await fetch(`/game.php?screen=info_player&id=${radarTargetPlayerId}`);
                if (res.ok) {
                    const text = await res.text();
                    const doc = new DOMParser().parseFromString(text, 'text/html');
                    const vLinks = doc.querySelectorAll('a[href*="screen=info_village"]');
                    const seenIds = new Set();
                    vLinks.forEach(a => {
                        const m = a.href.match(/id=(\d+)/);
                        if (m && !seenIds.has(m[1])) {
                            seenIds.add(m[1]);
                            const coordMatch = a.textContent.match(/\d{1,3}\|\d{1,3}/);
                            targetVillages.push({
                                id: m[1],
        ...
        if (targetVillages.length === 0) {
            showToast(`⚠️ Nenhuma aldeia encontrada para "${radarTargetPlayer}".`);
            return;
        }

        radarIsScanning = true;
        radarScanAbort = false;
        radarResults = [];
        const container = document.getElementById('tw-main-body');
        if (container) renderRadarIntel(container);

        const updateProgress = (phase, current, total) => {
            const pct = total > 0 ? Math.round((current / total) * 100) : 0;
            const pText = document.getElementById('tw-radar-progress-text');
            const pPct = document.getElementById('tw-radar-progress-pct');
            const pBar = document.getElementById('tw-radar-progress-bar');
            if (pText) pText.textContent = `${phase} (${current}/${total})...`;
            if (pPct) pPct.textContent = `${pct}%`;
            if (pBar) pBar.style.width = `${pct}%`;
        };

        const villageIds = targetVillages.map(v => v.id);
        const discoveredCmds = [];
        const seenCmdIds = new Set();
        const batchSize = 5;

        // FASE 1: Varredura de Aldeias (Tooltips)
        for (let i = 0; i < villageIds.length; i += batchSize) {
            if (radarScanAbort) break;
            const batch = villageIds.slice(i, i + batchSize);
            const current = Math.min(i + batchSize, villageIds.length);
            updateProgress('A extrair comandos das aldeias', current, villageIds.length);

            await Promise.all(batch.map(async (vId) => {
                if (radarScanAbort) return;
                try {
                    const res = await fetch(`/game.php?screen=info_village&id=${vId}`);
                    if (!res.ok) return;
                    const html = await res.text();
                    const doc = new DOMParser().parseFromString(html, 'text/html');

                    const knownV = targetVillages.find(v => String(v.id) === String(vId));
                    let destName = knownV ? `${knownV.name} (${knownV.coord})` : `Aldeia ID ${vId}`;
                    let destCoord = knownV ? knownV.coord : '';

                    if (!destCoord) {
                        const tds = doc.querySelectorAll('#content_value table.vis td');
                        for (let j = 0; j < tds.length; j++) {
                            if (tds[j].textContent.trim() === 'Coordenadas:' && tds[j + 1]) {
                                const cm = tds[j + 1].textContent.match(/(\d{1,3}\|\d{1,3})/);
                                if (cm) {
                                    destCoord = cm[1];
                    ...
                    links.forEach(l => {
                        ...
                        const troops = parseTroopsFromHtmlOrDoc(tooltipHtml);
                        discoveredCmds.push({
                            id: cmdId,
                            destName: destName,
                            destCoord: destCoord,
        ...
        if (radarScanAbort) {
            ...
            showToast('⏹️ Varredura cancelada.');
            if (container) renderRadarIntel(container);
            return;
        }
        ...
        const processedCmds = [];
        if (radarDeepScan && discoveredCmds.length > 0) {
            for (let i = 0; i < discoveredCmds.length; i += batchSize) {
                if (radarScanAbort) break;
                const batch = discoveredCmds.slice(i, i + batchSize);
                const current = Math.min(i + batchSize, discoveredCmds.length);
                updateProgress('A analisar detalhes dos comandos', current, discoveredCmds.length);

                const batchResults = await Promise.all(batch.map(async (partialCmd) => {
                    if (radarScanAbort) return null;
                    return await fetchDeepCommandInfo(partialCmd);
                }));

                batchResults.forEach(r => {
                    if (r) processedCmds.push(r);
                });
            ...
            // Varredura Rápida
            discoveredCmds.forEach(c => {
                const isAtk = !c.tooltipHtml.includes('support');
                const cmdType = isAtk ? 'Ataque' : 'Apoio';
                const classification = classifyRadarThreat(cmdType, c.tooltipTroops, 'Comando', c.tooltipHtml);
                processedCmds.push({
                    id: c.id,
                    text: 'Comando',
                    type: cmdType,
                    ...
                    destCoord: c.destCoord,
                    villageId: c.villageId,
                    origin: '-------',
                    player: radarTargetPlayer,
                    endTime: 0,
                    arrivalStr: '-',
                    timerStr: '--:--:--',
                    troops: c.tooltipTroops
                });
            });
        }
        ...
        let finalResults = processedCmds;
        if (radarFilterSupport) {
            finalResults = finalResults.filter(c => c.type !== 'Apoio');
        }

        radarResults = finalResults;
        radarIsScanning = false;
        showToast(`✅ Varredura concluída: ${radarResults.length} comandos detetados!`);
        if (container) renderRadarIntel(container);
    }

    function renderRadarIntel(container) {
        if (!container) return;
        ...
        const snobCount = radarResults.filter(c => c.classificationLabel === 'Nobre').length;
        const nukeCount = radarResults.filter(c => c.classificationLabel === 'Grande Escala' || c.classificationLabel === 'Alvo Detetado').length;
        const fakeCount = radarResults.filter(c => c.classificationLabel === 'Fake').length;
        const spyCount = radarResults.filter(c => c.classificationLabel === 'Espionagem').length;
        const supportCount = radarResults.filter(c => c.type === 'Apoio').length;

        let filtered = [...radarResults];
        if (radarFilter === 'noble') filtered = filtered.filter(c => c.classificationLabel === 'Nobre');
        else if (radarFilter === 'nuke') filtered = filtered.filter(c => c.classificationLabel === 'Grande Escala' || c.classificationLabel === 'Alvo Detetado');
        else if (radarFilter === 'fake') filtered = filtered.filter(c => c.classificationLabel === 'Fake');
        else if (radarFilter === 'spy') filtered = filtered.filter(c => c.classificationLabel === 'Espionagem');
        else if (radarFilter === 'support') filtered = filtered.filter(c => c.type === 'Apoio');

        if (radarSearch) {
            const q = radarSearch.trim().toLowerCase();
            filtered = filtered.filter(c => {
                return (c.text && c.text.toLowerCase().includes(q)) ||
                       (c.origin && c.origin.includes(q)) ||
                       (c.player && c.player.toLowerCase().includes(q)) ||
                       (c.destination && c.destination.toLowerCase().includes(q)) ||
                       (c.destCoord && c.destCoord.includes(q)) ||
                       (c.classificationLabel && c.classificationLabel.toLowerCase().includes(q));
            });
        }

        const grouped = new Map();
        filtered.forEach(cmd => {
            const key = cmd.destCoord || cmd.destination || 'Desconhecido';
            if (!grouped.has(key)) grouped.set(key, []);
            grouped.get(key).push(cmd);
        });

        for (const [key, cmds] of grouped.entries()) {
            cmds.sort((a, b) => {
                if (a.isPriority && !b.isPriority) return -1;
                if (!a.isPriority && b.isPriority) return 1;
                return (a.endTime || 999999999999) - (b.endTime || 999999999999);
            });
        }

        const targetVillagesCount = radarTargetPlayerId ? ((worldVillages || []).filter(v => String(v.playerId) === String(radarTargetPlayerId)).length) : 0;

        container.innerHTML = `
            <div style="display:flex; flex-direction:column; gap:10px; height:100%; overflow:hidden;">
                <!-- SUB-TAB SWITCHER -->
                <div style="display:flex; justify-content:space-between; align-items:center; background:#0f172a; border:1px solid #1e293b; border-radius:8px; padding:6px 12px; flex-wrap:wrap; gap:8px;">
                    <div style="display:flex; gap:6px;">
                        <button type="button" class="tw-btn" id="tw-btn-subtab-my-commands" style="padding:4px 14px; font-size:11.5px; font-weight:700; background:#1e293b; color:#94a3b8; border:1px solid #334155;">
                            🛡️ Meus Comandos & Retornos
                        </button>
                        <button type="button" class="tw-btn tw-btn-blue" id="tw-btn-subtab-radar" style="padding:4px 14px; font-size:11.5px; font-weight:700; background:#0284c7; border-color:#0369a1; color:#fff; box-shadow:0 0 10px rgba(2,132,199,0.35);">
                            📡 Radar Inimigo & Intel <span style="font-size:9.5px; background:rgba(239,68,68,0.25); color:#fca5a5; padding:1px 5px; border-radius:6px; margin-left:4px; font-weight:bold; border:1px solid rgba(239,68,68,0.4);">RADAR</span>
                        </button>
                    </div>
                    <div style="font-size:11px; color:#64748b;">
                        Espionagem e rastreio avançado de movimentações de jogadores do mundo
                    </div>
                </div>

                <!-- SEARCH & CONFIGURATION BAR -->
                <div style="background:#0f172a; border:1px solid #1e293b; border-radius:8px; padding:12px 14px; display:flex; flex-direction:column; gap:10px;">
                    <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px;">
                        <div style="position:relative; flex:1; min-width:280px; max-width:440px;">
                            <label style="display:block; font-size:11px; font-weight:700; color:#94a3b8; text-transform:uppercase; margin-bottom:4px;">
                                🎯 Jogador Alvo para Rastreio:
                            </label>
                            <div style="position:relative;">
                                <input type="text" id="tw-radar-player-input" class="tw-input" style="width:100%; padding:7px 10px; font-size:12.5px; background:#020617; border:1px solid #334155; color:#f8fafc; border-radius:6px; box-sizing:border-box;" placeholder="Digita o nome do jogador (ex: Inimigo123)..." autocomplete="off" value="${escapeHtml(radarTargetPlayer)}">
                                <div id="tw-radar-player-dropdown" style="display:none; position:absolute; top:calc(100% + 4px); left:0; right:0; background:#0f172a; border:1px solid #38bdf8; border-radius:6px; max-height:220px; overflow-y:auto; z-index:999999; box-shadow:0 10px 25px rgba(0,0,0,0.85);"></div>
                            </div>
                            <div id="tw-radar-target-feedback" style="font-size:11px; color:#38bdf8; margin-top:4px;">
                                ${radarTargetPlayerId ? `✅ Alvo ativo: <b>${escapeHtml(radarTargetPlayer)}</b> (${targetVillagesCount} aldeias no mundo)` : 'Digita para pesquisar em tempo real qualquer jogador do mundo.'}
                            </div>
                        </div>

                        <div style="display:flex; align-items:center; gap:16px; flex-wrap:wrap;">
                            <label style="display:flex; align-items:center; gap:6px; font-size:11.5px; color:#cbd5e1; cursor:pointer; user-select:none;">
                                <input type="checkbox" id="tw-radar-opt-deep" ${radarDeepScan ? 'checked' : ''} style="cursor:pointer;">
                                <span>🔬 <b>Varredura Profunda</b> (Origens, Tropas e Segundos)</span>
                            </label>
                            <label style="display:flex; align-items:center; gap:6px; font-size:11.5px; color:#cbd5e1; cursor:pointer; user-select:none;">
                                <input type="checkbox" id="tw-radar-opt-filter-support" ${radarFilterSupport ? 'checked' : ''} style="cursor:pointer;">
                                <span>⚔️ <b>Apenas Ataques</b> (Ocultar Apoios)</span>
                            </label>
                        </div>

                        <div style="display:flex; gap:8px;">
                            <button type="button" class="tw-btn tw-btn-blue" id="tw-btn-radar-start" style="padding:7px 18px; font-size:12px; font-weight:bold; display:flex; align-items:center; gap:6px;">
                                ${radarIsScanning ? '<span class="tw-spinner" style="width:14px; height:14px; border-width:2px;"></span> A Rastrear...' : '🚀 Iniciar Radar de Intel'}
                            </button>
                            ${radarIsScanning ? `
                            <button type="button" class="tw-btn" id="tw-btn-radar-abort" style="background:#dc2626; color:#fff; border:none; padding:7px 14px; font-size:12px; font-weight:bold; cursor:pointer;">
                                ⏹️ Interromper
                            </button>
                            ` : ''}
                        </div>
                    ...
                    <!-- PROGRESS BAR -->
                    <div id="tw-radar-progress-container" style="display:${radarIsScanning ? 'block' : 'none'}; background:#020617; border:1px solid #1e293b; border-radius:6px; padding:8px 12px;">
                        <div style="display:flex; justify-content:space-between; font-size:11px; margin-bottom:4px;">
                            <span id="tw-radar-progress-text" style="color:#38bdf8; font-weight:600;">A inicializar varredura...</span>
                            <span id="tw-radar-progress-pct" style="color:#94a3b8; font-family:monospace;">0%</span>
                        </div>
                        <div style="width:100%; height:6px; background:#1e293b; border-radius:3px; overflow:hidden;">
                            <div id="tw-radar-progress-bar" style="width:0%; height:100%; background:linear-gradient(90deg, #0284c7, #38bdf8); transition:width 0.15s ease;"></div>
                        </div>
                    </div>
                </div>
                ...
                <div style="display:grid; grid-template-columns: repeat(6, 1fr); gap:8px;">
                    <div class="tw-kpi-card" style="border:1px solid #9333ea; background:#0f172a;">
                        <div class="tw-kpi-label" style="color:#c084fc;">NOBRES DETETADOS <span>👑</span></div>
                        <div class="tw-kpi-value" style="color:#e9d5ff;">${snobCount}</div>
                        <div class="tw-kpi-sub">Ameaça máxima</div>
                    </div>
                    <div class="tw-kpi-card" style="border:1px solid #dc2626; background:#0f172a;">
                        <div class="tw-kpi-label" style="color:#f87171;">GRANDE ESCALA / NUKES <span>⚔️</span></div>
                        <div class="tw-kpi-value" style="color:#fecaca;">${nukeCount}</div>
                        <div class="tw-kpi-sub">Ataques pesados (>200 pop)</div>
                    </div>
                    <div class="tw-kpi-card" style="border:1px solid #d97706; background:#0f172a;">
                        <div class="tw-kpi-label" style="color:#fbbf24;">FAKES DETETADOS <span>🗡️</span></div>
                        <div class="tw-kpi-value" style="color:#fef3c7;">${fakeCount}</div>
                        <div class="tw-kpi-sub">Ataques leves (<=200 pop)</div>
                    </div>
                    <div class="tw-kpi-card" style="border:1px solid #2563eb; background:#0f172a;">
                        <div class="tw-kpi-label" style="color:#60a5fa;">ESPIONAGENS <span>👁️</span></div>
                        <div class="tw-kpi-value" style="color:#dbeafe;">${spyCount}</div>
                        <div class="tw-kpi-sub">Apenas batedores</div>
                    </div>
                    <div class="tw-kpi-card" style="border:1px solid #059669; background:#0f172a;">
                        <div class="tw-kpi-label" style="color:#34d399;">APOIOS DETETADOS <span>🛡️</span></div>
                        <div class="tw-kpi-value" style="color:#d1fae5;">${supportCount}</div>
                        <div class="tw-kpi-sub">Reforços a caminho</div>
                    </div>
                    <div class="tw-kpi-card tw-kpi-blue">
                        <div class="tw-kpi-label">TOTAL COMANDOS <span>🎯</span></div>
                        <div class="tw-kpi-value">${radarResults.length}</div>
                        <div class="tw-kpi-sub">Ordens ativas do alvo</div>
                    </div>
                </div>
                ...
                <div style="display:flex; justify-content:space-between; align-items:center; background:#1e293b; padding:8px 12px; border-radius:6px; border:1px solid #334155; flex-wrap:wrap; gap:8px;">
                    <div style="display:flex; gap:6px; align-items:center; flex-wrap:wrap;" id="tw-radar-filter-pills">
                        <span style="font-size:11px; color:#94a3b8; font-weight:bold; margin-right:4px;">FILTRO:</span>
                        <div class="tw-pill ${radarFilter==='all'?'active':''}" data-rf="all">Todos (${radarResults.length})</div>
                        <div class="tw-pill ${radarFilter==='noble'?'active':''}" data-rf="noble" style="${snobCount > 0 ? 'background:rgba(147,51,234,0.25); color:#e9d5ff; border:1px solid #9333ea; font-weight:bold;' : ''}">👑 Nobres (${snobCount})</div>
                        <div class="tw-pill ${radarFilter==='nuke'?'active':''}" data-rf="nuke" style="${nukeCount > 0 ? 'background:rgba(220,38,38,0.25); color:#fca5a5; border:1px solid #dc2626; font-weight:bold;' : ''}">⚔️ Grande Escala (${nukeCount})</div>
                        <div class="tw-pill ${radarFilter==='fake'?'active':''}" data-rf="fake">🗡️ Fakes (${fakeCount})</div>
                        <div class="tw-pill ${radarFilter==='spy'?'active':''}" data-rf="spy">👁️ Espionagens (${spyCount})</div>
                        <div class="tw-pill ${radarFilter==='support'?'active':''}" data-rf="support">🛡️ Apoios (${supportCount})</div>
                    </div>

                    <div style="display:flex; gap:8px; align-items:center;">
                        <input type="text" id="tw-radar-search" class="tw-input" style="width:160px;" placeholder="🔍 Filtrar resultados..." value="${escapeHtml(radarSearch)}">
                        <button type="button" class="tw-btn" id="tw-btn-radar-copy-bb" style="padding:5px 12px; font-size:11px; background:#0284c7; color:#fff; border-color:#0369a1; font-weight:bold;" title="Copiar relatório formatado em BBCode">
                            📋 Copiar BBCode
                        </button>
                        <button type="button" class="tw-btn" id="tw-btn-radar-clear" style="padding:5px 10px; font-size:11px; background:#1e293b; color:#94a3b8; border-color:#334155;" title="Limpar resultados">
                            🗑️ Limpar
                        </button>
                    </div>
                </div>
                ...
                <div class="tw-panel" style="flex-grow:1; overflow-y:auto;">
                    ${radarResults.length === 0 ? `
                    <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; height:100%; min-height:300px; gap:12px; color:#64748b;">
                        <div style="font-size:36px;">📡</div>
                        <div style="font-size:14px; font-weight:700; color:#cbd5e1;">Nenhum comando em visualização</div>
                        <div style="font-size:12px; color:#94a3b8; max-width:440px; text-align:center; line-height:1.5;">
                            ${radarIsScanning ? 'A varrer o jogador alvo... Acompanha o progresso acima.' : 'Seleciona um jogador acima e clica em <b>Iniciar Radar de Intel</b> para extrair todas as movimentações ativas.'}
                        </div>
                    </div>
                    ` : (grouped.size === 0 ? `
                    <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; height:100%; min-height:250px; gap:8px; color:#64748b;">
                        <div style="font-size:28px;">🔍</div>
                        <div style="font-size:13px; font-weight:bold; color:#cbd5e1;">Nenhum comando coincide com os filtros atuais</div>
                        <div style="font-size:11px; color:#94a3b8;">Experimenta selecionar o filtro "Todos" ou limpar o termo de pesquisa.</div>
                    </div>
                    ` : `
                    <div id="tw-radar-accordion-container" style="display:flex; flex-direction:column; gap:6px;">
                        ${Array.from(grouped.entries()).map(([destKey, groupCmds]) => {
                            const firstCmd = groupCmds[0] || {};
                            const isCollapsed = collapsedRadarGroups.has(destKey);
                            const priorCount = groupCmds.filter(c => c.isPriority).length;
                            const villageId = firstCmd.villageId;
                            const destCoord = firstCmd.destCoord;
                            const destName = firstCmd.destination;

                            return `
                                <div class="tw-radar-group-card" style="border:1px solid #334155; border-radius:6px; overflow:hidden; background:#0b1120;">
                                    <div class="tw-radar-group-header" data-dest-key="${escapeHtml(destKey)}" style="display:flex; justify-content:space-between; align-items:center; background:linear-gradient(90deg, #1e293b 0%, #0f172a 100%); padding:8px 12px; border-bottom:1px solid ${priorCount > 0 ? '#ef4444' : '#334155'}; cursor:pointer; user-select:none;">
                                        <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
                                            <span class="tw-radar-toggle-icon" style="font-size:11px; font-weight:bold; color:#38bdf8; width:12px;">${isCollapsed ? '▶' : '▼'}</span>
                                            <span style="font-size:12.5px; font-weight:700; color:#f8fafc;">🎯 Destino: <b style="color:#38bdf8;">${escapeHtml(destName)}</b></span>
                                            ${priorCount > 0 ? `<span style="font-size:10px; font-weight:bold; background:#dc2626; color:#fff; padding:2px 7px; border-radius:4px; box-shadow:0 0 6px rgba(220,38,38,0.5);">⚠️ ${priorCount} Alerta(s)</span>` : ''}
                                            <span style="font-size:11px; color:#94a3b8;">(${groupCmds.length} ${groupCmds.length === 1 ? 'comando' : 'comandos'})</span>
                                        </div>
                                        <div style="display:flex; gap:8px; align-items:center;" onclick="event.stopPropagation();">
                                            ${villageId ? `<a href="/game.php?screen=info_village&id=${villageId}" target="_blank" class="tw-btn" style="text-decoration:none; padding:3px 10px; font-size:11px; background:#1e293b; border:1px solid #334155; color:#38bdf8; border-radius:4px;">Ver In-Game ↗</a>` : ''}
                                        </div>
                                    </div>
                                    <div class="tw-radar-group-body" style="display:${isCollapsed ? 'none' : 'block'};">
                                        <table class="tw-table">
                                            <thead>
                                                <tr>
                                                    <th style="text-align:left; width:220px; padding-left:12px;">Comando</th>
                                                    <th style="width:140px; text-align:center;">Classificação</th>
                                                    <th style="width:130px; text-align:center;">Origem</th>
                                                    <th style="width:140px; text-align:center;">Jogador</th>
                                                    <th style="width:160px; text-align:center;">Chegada</th>
                                                    <th style="width:120px; text-align:right; padding-right:12px;">Tempo Restante</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                ${groupCmds.map((cmd) => {
                                                    const icon = cmd.type === 'Apoio' ? 'support.png' : 'attack.png';
                                                    const iconHtml = `<img src="https://dspt.innogamescdn.com/asset/1057e93c/graphic/command/${icon}" style="vertical-align:-2px; margin-right:4px;">`;
                                                    const cmdUrl = `/game.php?screen=info_command&id=${cmd.id}`;
                                                    const btnCmd = `<a href="${cmdUrl}" target="_blank" style="margin-left:6px; font-size:9.5px; background:#1e293b; border:1px solid #334155; padding:1px 5px; border-radius:3px; text-decoration:none; color:#38bdf8;" title="Ver Comando In-Game">🔍</a>`;

                                                    return `
                                                        <tr style="${cmd.isPriority ? 'background:rgba(239,68,68,0.06); border-left:3px solid #ef4444;' : 'border-left:3px solid #334155;'}">
                                                            <td style="text-align:left; padding-left:12px;">
                                                                <div style="display:flex; align-items:center;">
                                                                    ${iconHtml} <b style="color:#f8fafc; font-size:11.5px;">${escapeHtml(cmd.text)}</b> ${btnCmd}
                                                                </div>
                                                            </td>
                                                            <td style="text-align:center;">${cmd.badgeHtml}</td>
                                                            <td style="text-align:center;">
                                                                <span style="font-family:monospace; font-weight:600; color:#38bdf8;">${cmd.origin || '-------'}</span>
                                                                ${cmd.origin && cmd.origin !== '-------' ? `<button type="button" class="tw-btn tw-cmd-copy-coord" data-coord="${cmd.origin}" style="padding:1px 5px; font-size:9.5px; margin-left:4px;" title="Copiar coordenada">📋</button>` : ''}
                                                            </td>
                                                            <td style="text-align:center; color:#cbd5e1; font-weight:500;">${escapeHtml(cmd.player || '-------')}</td>
                                                            <td style="text-align:center; color:#94a3b8; font-size:11px;">${escapeHtml(cmd.arrivalStr || '-')}</td>
                                                            <td style="text-align:right; padding-right:12px;">
                                                                <span class="tw-radar-timer" data-endtime-sec="${cmd.endTime ? Math.floor(cmd.endTime / 1000) : 0}" style="font-family:monospace; font-size:12.5px; font-weight:bold; color:${cmd.isPriority ? '#f87171' : '#38bdf8'}; background:#020617; padding:2px 6px; border-radius:4px; border:1px solid #334155;">
                                                                    ${cmd.timerStr || '--:--:--'}
                                                                </span>
                                                            </td>
                                                        </tr>
                ...
                <div style="display:flex; justify-content:space-between; align-items:center; padding:2px 4px; font-size:12px; color:#94a3b8;">
                    <span>A apresentar <b>${filtered.length}</b> de <b>${radarResults.length}</b> comandos detetados</span>
                    <span style="font-size:11px; color:#64748b;">Radar Intel TW • Rate-limiting seguro com lotes assíncronos</span>
                </div>
            </div>
        `;

        // Subtab event listeners
        const btnSubtabMyCmds = document.getElementById('tw-btn-subtab-my-commands');
        if (btnSubtabMyCmds) {
            btnSubtabMyCmds.addEventListener('click', () => {
                commandsSubTab = 'my_commands';
                renderCommands();
            });
        }

        // Radar Player Input Autocomplete
        const pInput = document.getElementById('tw-radar-player-input');
        const pDropdown = document.getElementById('tw-radar-player-dropdown');
        const pFeedback = document.getElementById('tw-radar-target-feedback');

        if (pInput && pDropdown) {
            let activeIndex = -1;

            const updateDropdown = () => {
                const term = pInput.value.trim().toLowerCase();
                if (!term || !worldPlayersLoaded || !worldPlayers) {
                ...
                for (const [id, name] of Object.entries(worldPlayers)) {
                    if (name.toLowerCase().includes(term)) {
                        const vCount = (worldVillages || []).filter(v => String(v.playerId) === String(id)).length;
                        matches.push({ id, name, vCount });
                        if (matches.length >= 10) break;
                    }
                }

                if (matches.length === 0) {
                    pDropdown.innerHTML = `<div style="padding:8px 12px; color:#64748b; font-size:11.5px;">Nenhum jogador encontrado com "${escapeHtml(term)}"</div>`;
                    pDropdown.style.display = 'block';
                    return;
                }

                activeIndex = -1;
                pDropdown.innerHTML = matches.map((m, idx) => `
                    <div class="tw-radar-dropdown-item" data-idx="${idx}" data-pid="${m.id}" data-pname="${escapeHtml(m.name)}" data-vcount="${m.vCount}" style="padding:7px 12px; cursor:pointer; display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid #1e293b; font-size:12px; transition:background 0.1s;">
                        <span style="color:#f8fafc; font-weight:600;">👤 ${escapeHtml(m.name)}</span>
                        <span style="font-size:10px; color:#38bdf8; background:rgba(56,189,248,0.15); padding:1px 6px; border-radius:4px;">${m.vCount} aldeias</span>
                    </div>
                `).join('');
                pDropdown.style.display = 'block';

                pDropdown.querySelectorAll('.tw-radar-dropdown-item').forEach(item => {
                    item.addEventListener('mouseenter', function() {
                        pDropdown.querySelectorAll('.tw-radar-dropdown-item').forEach(i => i.style.background = 'transparent');
                        this.style.background = '#1e293b';
                    });
                    item.addEventListener('click', function() {
                        selectPlayer(this.getAttribute('data-pid'), this.getAttribute('data-pname'), this.getAttribute('data-vcount'));
                    });
                });
            };

            const selectPlayer = (id, name, vCount) => {
                radarTargetPlayerId = id;
                radarTargetPlayer = name;
                pInput.value = name;
                pDropdown.style.display = 'none';
                if (pFeedback) {
                    pFeedback.innerHTML = `✅ Alvo ativo: <b>${escapeHtml(name)}</b> (${vCount} aldeias no mundo)`;
                }
            };

            pInput.addEventListener('input', updateDropdown);
            pInput.addEventListener('focus', () => {
                if (pInput.value.trim().length > 0) updateDropdown();
            });

            pInput.addEventListener('keydown', (e) => {
                const items = pDropdown.querySelectorAll('.tw-radar-dropdown-item');
                if (pDropdown.style.display !== 'block' || items.length === 0) return;

                if (e.key === 'ArrowDown') {
                    e.preventDefault();
                    activeIndex = (activeIndex + 1) % items.length;
                    items.forEach((i, idx) => i.style.background = idx === activeIndex ? '#1e293b' : 'transparent');
            ...
            document.addEventListener('click', (e) => {
                if (!pInput.contains(e.target) && !pDropdown.contains(e.target)) {
                    pDropdown.style.display = 'none';
                }
            });
        }
        ...
        // Options Checkboxes
        const optDeep = document.getElementById('tw-radar-opt-deep');
        if (optDeep) {
            optDeep.addEventListener('change', (e) => {
                radarDeepScan = e.target.checked;
            });
        }

        const optSupport = document.getElementById('tw-radar-opt-filter-support');
        if (optSupport) {
            optSupport.addEventListener('change', (e) => {
                radarFilterSupport = e.target.checked;
                if (radarResults.length > 0) {
                    renderRadarIntel(container);
                }
            });
        }

        // Start / Abort Buttons
        const btnStart = document.getElementById('tw-btn-radar-start');
        if (btnStart) {
        ...
        const btnAbort = document.getElementById('tw-btn-radar-abort');
        if (btnAbort) {
            btnAbort.addEventListener('click', () => {
                radarScanAbort = true;
                showToast('⏹️ A interromper varredura...');
            });
        }

        // Filter pills
        document.querySelectorAll('#tw-radar-filter-pills [data-rf]').forEach(pill => {
            pill.addEventListener('click', function() {
                radarFilter = this.getAttribute('data-rf');
                renderRadarIntel(container);
            });
        });

        // Search within results
        const rSearch = document.getElementById('tw-radar-search');
        if (rSearch) {
            rSearch.addEventListener('input', (e) => {
                radarSearch = e.target.value.toLowerCase();
                renderRadarIntel(container);
            });
        }

        // Copy BBCode
        const btnCopyBb = document.getElementById('tw-btn-radar-copy-bb');
        if (btnCopyBb) {
            btnCopyBb.addEventListener('click', () => {
                const bb = generateRadarBbCode(filtered);
                if (bb) {
                    safeCopyText(bb);
                    showToast('📋 Relatório BBCode copiado com sucesso!');
                } else {
                    showToast('⚠️ Nenhum comando para exportar.');
                }
            });
        }

        // Clear Results
        const btnClear = document.getElementById('tw-btn-radar-clear');
        if (btnClear) {
            btnClear.addEventListener('click', () => {
                radarResults = [];
                renderRadarIntel(container);
                showToast('🗑️ Resultados do radar limpos.');
            });
        }

        // Accordion Headers
        document.querySelectorAll('.tw-radar-group-header').forEach(header => {
            header.addEventListener('click', function() {
                const key = this.getAttribute('data-dest-key');
                if (key) {
                    if (collapsedRadarGroups.has(key)) collapsedRadarGroups.delete(key);
                    else collapsedRadarGroups.add(key);
                    renderRadarIntel(container);
                }
            });
        });
        ...
        // Copy Coord Buttons
        document.querySelectorAll('.tw-cmd-copy-coord').forEach(btn => {
    ...
    function renderCommands() {
        ...
        container.innerHTML = `
            <div style="display:flex; flex-direction:column; gap:10px; height:100%; overflow:hidden;">
                <!-- SUB-TAB SWITCHER: MINHA CONTA VS RADAR TÁTICO -->
                <div style="display:flex; justify-content:space-between; align-items:center; background:#0f172a; border:1px solid #1e293b; border-radius:8px; padding:6px 12px; flex-wrap:wrap; gap:8px;">
                    <div style="display:flex; gap:6px;">
                        <button type="button" class="tw-btn ${commandsSubTab === 'my_commands' ? 'tw-btn-blue' : ''}" id="tw-btn-subtab-my-commands" style="padding:4px 14px; font-size:11.5px; font-weight:700; ${commandsSubTab === 'my_commands' ? 'box-shadow:0 0 8px rgba(56,189,248,0.3);' : 'background:#1e293b; color:#94a3b8; border:1px solid #334155;'}">
                            🛡️ Meus Comandos & Retornos
                        </button>
                        <button type="button" class="tw-btn" id="tw-btn-subtab-radar" style="padding:4px 14px; font-size:11.5px; font-weight:700; background:#1e293b; color:#94a3b8; border:1px solid #334155;">
                            📡 Radar Inimigo & Intel <span style="font-size:9.5px; background:rgba(239,68,68,0.25); color:#fca5a5; padding:1px 5px; border-radius:6px; margin-left:4px; font-weight:bold; border:1px solid rgba(239,68,68,0.4);">RADAR</span>
                        </button>
                    </div>
                    <div style="font-size:11px; color:#64748b;">
                        Monitorização de ataques recebidos, retornos de tropas e comandos ativos da conta
                    </div>
                </div>
        ...
        // Event listeners
        const btnSubtabRadar = document.getElementById('tw-btn-subtab-radar');
        if (btnSubtabRadar) {
            btnSubtabRadar.addEventListener('click', () => {
                commandsSubTab = 'radar_intel';
                renderCommands();
            });
        }
    ...
    function closeSuite() {
        ...
        if (radarTimerInterval) { clearInterval(radarTimerInterval); radarTimerInterval = null; }
        if (ui) ui.remove();
